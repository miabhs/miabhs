...
import id.my.mdn.kupu.app.pengajaran.view.widget.PencapaianBelajarList;
 ...
 */
@Named(value = "pencapaianBelajarPage")
@ViewScoped
public class PencapaianBelajarPage extends Page implements Serializable {
    ...
    @Inject @Bookmarked
    private PencapaianBelajarList dataView;
    ...
    @Creator(of = "dataView")
    public void create() {
        PencapaianBelajar pencapaian = new PencapaianBelajar();
        pencapaian.setPengajaranDate(LocalDate.now());
        dataView.create(pencapaian);
    }
    ...
    public void saveEditSantri(AjaxBehaviorEvent evt) {
        PencapaianBelajar pencapaianBelajar = dataView.getFetchedItems().get(activeIndex);
        pencapaianBelajar.setSantri(choosenSantri);
        dataView.edit(pencapaianBelajar);
        PrimeFaces.current().ajax().update(listUpdate);
    }

    public void saveEditKitab(AjaxBehaviorEvent evt) {
        PencapaianBelajar pencapaianBelajar = dataView.getFetchedItems().get(activeIndex);
        pencapaianBelajar.setJenisKitab(choosenKitab);        
        dataView.edit(pencapaianBelajar);
        PrimeFaces.current().ajax().update(listUpdate);
    }
    ...
    public PencapaianBelajarList getDataView() {
        return dataView;
    }
