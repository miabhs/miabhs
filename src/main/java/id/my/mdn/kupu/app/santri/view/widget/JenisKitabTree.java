 ...
 */
@Named(value = "jenisKitabTree")
@Dependent
public class JenisKitabTree extends AbstractMutableTree<JenisKitab> {
    ...
    protected List<JenisKitab> getFetchedItemsInternal(Map<String, Object> parameters, List<FilterData> filters, List<SorterData> sorters, DefaultList<JenisKitab> defaultList, AbstractFacade.DefaultChecker defaultChecker) {
        return dao.findAll(0, 0, parameters, filters, sorters);
    }
    ...
    private final JenisKitabTree that = this;
        ...
        if (model == null) {
            model = new LazyDataModel<JenisKitab>() {
                ...
                public List<JenisKitab> load(int first, int pageSize, Map<String, SortMeta> map, Map<String, FilterMeta> map1) {
                    return that.getFetchedItemsInternal(
